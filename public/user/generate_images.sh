...
# Convert .jpeg to .png if any exist
for file in *.jpeg; do
    [ -f "$file" ] && mv "$file" "${file%.jpeg}.png"
done

# Define Source Arrays
male_sources=(brother*.png man*.png image-profile*.png)
female_sources=(girl*.png sister*.png)
...
get_random_image() {
    ...
    if [ "$gender" == "male" ]; then
        ...
        if [ ${#male_sources[@]} -eq 0 ]; then echo ""; return; fi
        local rand_idx=$(( RANDOM % ${#male_sources[@]} ))
        echo "${male_sources[$rand_idx]}"
    else
        # Pick random female image
        if [ ${#female_sources[@]} -eq 0 ]; then echo ""; return; fi
        local rand_idx=$(( RANDOM % ${#female_sources[@]} ))
        echo "${female_sources[$rand_idx]}"
    fi
}
...
for target in "${targets[@]}"; do
    if [[ "$target" == *"sister"* || "$target" == *"mother"* || "$target" == *"girl"* || "$target" == *"aunty"* || "$target" == *"niece"* || "$target" == *"grandmother"* ]]; then
        source_img=$(get_random_image "female")
    else
        source_img=$(get_random_image "male")
    fi

    if [ -f "$source_img" ]; then
        cp "$source_img" "final_assets/$target"
        echo "Created $target"
    else
        echo "Warning: No source image found for $target (Source was: $source_img)"
    fi
done

echo "Done! Check final_assets folder."
